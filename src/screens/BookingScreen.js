        ...
        <DefaultTemplate>
            <Container>
                ...
                <Row className={date ? 'pb-3' : 'pb-3 d-none'}>
                    <Col className={'container_date pb-3 container_date border-4 border-black border-bottom'}>
                        {
                            showTimes.map(s => (
                                <p key={s.id} className={s.id === time.id ? 'data_date border_active' : 'data_date'} onClick={(e) => setTime(s)}>{s.time}</p>
                            ))
                        }
                    </Col>
                </Row>
                <Row className={seats.length === 0 ? "d-none" : "pb-5 d-flex justify-content-center"}>
                    <Col xs={3} className="text-center border-5 border-danger border-bottom screen_room_area">
